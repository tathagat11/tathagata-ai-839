...
async def get_logs(
    ...
    level: Optional[str] = Query(
        None, description="Log level filter (INFO, ERROR, etc.)"
    ),
    start_date: Optional[str] = Query(
        None, description="Start date for log filtering (YYYY-MM-DD)"
    ),
    end_date: Optional[str] = Query(
        None, description="End date for log filtering (YYYY-MM-DD)"
    ),
) -> List[Dict]:
    try:
        log_entries = []
        parsing_errors = []
        with open("model_usage.log") as log_file:
            for line_number, line in enumerate(log_file, 1):
                try:
                    ...
                    if (
                        level
                        and log_entry.get("levelname", "").upper() != level.upper()
                    ):
                        continue
        ...
        if parsing_errors:
            return {"log_entries": log_entries, "parsing_errors": parsing_errors}
        return log_entries
    except Exception as e:
